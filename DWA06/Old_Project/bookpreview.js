...
// Variable to store the currently selected genre
let CURRENTGENRE =
	'none';
// Variable to store the currently selected author
let CURRENTAUTHOR =
	'none';
// Variable to store the currently selected title
let CURRENTTITLE =
	'none';

/**
 * Event handler for handling click on a preview element.
 * It retrieves information from the clicked element and updates the overlay with the corresponding data.
 */
export const handlePreviewClick =
    ...
    */
		...
		overlayAuthor.innerHTML =
			bookAuthor;
		overlayImg.setAttribute(
			'src',
			bookImageLink
		);
 ...
 */
export const handleClickAversion =
	(event) => {
		const newPreview =
			event.target.closest(
				'.preview'
...
/**
 * Event handler used to create the previews. It is called on startup to create the previews,
 * and also called when a filter is applied to the list of books and the search button is clicked.
 */
export const createPreviews =
	(event) => {
		...
		if (
			...
			for (
				...
				if (
					...
					dataMessage.classList.add(
						'list__message_show'
					);
				} else {
					dataMessage.classList.remove(
						'list__message_show'
					);
				}
			}
        ...
        `;
			dataListButton.removeAttribute(
				'disabled'
			);
		} else {
			dataListButton.setAttribute(
				'disabled',
				true
			);
		}
		...
		const searchOverlayCancelButton =
			document.querySelector(
				'[data-search-cancel]'
			);
		searchOverlayCancelButton.click();
